...
def do_it( metadata_file ):
    ...
    log_path = "hubmapbags.log"
    if environ.get("CFDE-BDBAG-LOG") is not None:
        log_path = environ.get("CFDE-BDBAG-LOG")
    logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)-8s %(message)s',
                            datefmt='%a, %d %b %Y %H:%M:%S', filename= log_path , filemode='w')

    datasets = pd.read_csv( metadata_file )
    logging.info( 'Number of datasets found is ' + str(datasets.shape[0]) )
    ...
    for dataset in datasets.iterrows():
        ...
        logging.info('Preparing bag for dataset ' + data_directory )
        ...
        if Path(done).exists():
            logging.info('Checkpoint found. Avoiding computation. To re-compute erase file ' + done)
        elif Path(computing).exists():
            logging.info('Checkpoint found. Avoiding computation since another process is building this bag.')
        else:
            with open(computing, 'w') as file:
                pass

            logging.info('Creating checkpoint ' + computing)

            if status == 'new':
                logging.info('Dataset is not published. Aborting computation.')

            output_directory = assay_type + '-' + status + '-' + dataset['dataset_uuid']
            p = Path( output_directory )

            if p.exists() and p.is_dir():
                logging.info('Removing existing folder ' + output_directory)
                rmtree(p)
                logging.info('Creating folder ' + output_directory)
            ...
            logging.info('Making file_describes_subject.tsv')
            ...
            logging.info('Making file_describes_biosample.tsv')
            ...
            logging.info('Making files_in_collection.tsv')
            answer = files_in_collection.create_manifest( hubmap_id, data_directory )
            move( 'file_in_collection.tsv', output_directory )

            logging.info('Removing checkpoint ' + computing )
            remove(computing)

            logging.info('Creating final checkpoint ' + done )
