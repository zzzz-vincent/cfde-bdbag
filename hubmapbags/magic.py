...
def do_it( data_provider, metadata_file ):
    ...
    for dataset in datasets.iterrows():
        ...
        if Path(done).exists():
            ...
            print('Making primary_dcc_contact.tsv')
...
def build_it( data_provider, metadata_file ):
    datasets = pd.read_csv( metadata_file )
    print( 'Number of datasets found is ' + str(datasets.shape[0]) )
    ...
    for dataset in datasets.iterrows():
        ...
        if Path(done).exists():
            ...
            print('Making file.tsv')
            answer = files.create_manifest( data_provider, assay_type, data_directory, from_cache=from_cache )
            if answer:
                move( 'file.tsv', output_directory )
            ...
            print('Making file_describes_subject.tsv')
            file_describes_subject.create_manifest( donor_id, data_directory, , from_cache=from_cache )
            move( 'file_describes_subject.tsv', output_directory )
            ...
            print('Making file_describes_biosample.tsv')
            file_describes_biosample.create_manifest( biosample_id, data_directory, from_cache=from_cache )
            move( 'file_describes_biosample.tsv', output_directory )
            ...
            print('Making files_in_collection.tsv')
            answer = files_in_collection.create_manifest( hubmap_id, data_directory, from_cache=from_cache  )
            move( 'file_in_collection.tsv', output_directory )
